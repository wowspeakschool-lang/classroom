...
(function(){
  ...
  function step(d){
    var i = idx() + d;
    /* В Уроке 1 развилка: правильный ответ ведёт на 2-2A, неправильный — на 2-2B.
       В линейном списке они лежат подряд, поэтому стрелками была бы видна и вторая
       реплика — как будто похвалили и тут же поправили. Ветку «неверно» пропускаем. */
    if (linearOrder[i] === "2-2B") i += d;
