...
STORY = {
 1: dict(island="Ярмарка Слов", icon="🎪", nextKey="Острова Зеркал",
     ...
     outro="Ты вернул порядок всей ярмарке! Хозяева прилавков говорят спасибо и отдают тебе "
           "ключик от Острова Зеркал. Бутылочка уже в рюкзаке — в путь!"),
 2: dict(island="Остров Зеркал", icon="🪞", nextKey="Острова Друзей",
     intro="Ты открыл Остров Зеркал! Хорошо, что у тебя есть волшебная бутылочка — дорога сюда "
           "была длинной. Здесь живут зеркала-примерочные: загляни в зеркало и скажи, что у тебя "
           "уже есть, а чего пока нет.",
 ...
 3: dict(island="Остров Друзей", icon="👫", nextKey="Замка Дракона",
     intro="Сапоги-скороходы принесли тебя прямо на Остров Друзей! Здесь живут Бен и Мия. "
           "Ветер смешал их вещи в одну кучу — помоги разобрать, кому что.",
 ...
 4: dict(island="Замок Дракона", icon="🏰", nextKey=None,
     intro="Вот он, Замок Дракона! Ворота уже открыты, а Дракон ещё спит — колокольчик "
           "пригодится. Сначала собери своего героя и расскажи, во что он одет.",
...
def patch(n):
    ...
    if "markIslandDone" in s:
        # Экраны уже встроены — обновляем только тексты. Раньше скрипт просто
        # пропускал такой файл, и правки реплик молча не доезжали.
        st = STORY[n]
        for sid, text in (("map-in", st["intro"]), ("map-out", st["outro"])):
            pat = re.compile(r'(id: "%s".*?speech: )"(?:[^"\\]|\\.)*"' % sid, re.S)
            s, cnt = pat.subn(lambda m: m.group(1) + json_str(text), s, count=1)
            assert cnt == 1, "не найден экран %s в уроке %d" % (sid, n)
