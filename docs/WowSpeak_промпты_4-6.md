...
1. **Первой сгенерируйте картинку №1** (фон карты). Она задаёт стиль.
   Дальше **прикрепляйте её к каждому следующему запросу** и оставляйте в тексте
   строку «Match the art style of the attached image exactly».
...
3. Под каждым промптом строка «проверить» — что должно быть на картинке.
   ...
   перегенерируйте.
4. Размеры: где написано **горизонталь** — 1536×1024, остальное 1024×1024.
